...
import { ArrowLeft, Printer, Download, CheckCircle2, Edit2, Save, X } from 'lucide-react';
...
export default function TaxInvoiceDetails() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        const allData = await fetchTaxInvoices();
        const found = allData.find(ti => ti.id === id);
  ...
  const handleSave = async () => {
    setIsEditing(false);
    try {
      const { fetchTaxInvoices, saveTaxInvoice } = await import('@/utils/api');
      const allData = await fetchTaxInvoices();
      const existing = allData.find(ti => ti.id === docNo) || { id: docNo };
    ...
    <div style={{ maxWidth: '900px', margin: '0 auto', paddingBottom: '60px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <div>
            <h1 style={{ fontSize: '1.8rem', fontWeight: 'bold', color: 'var(--text-main)', margin: '0 0 4px 0' }}>รายละเอียดใบเสร็จรับเงิน / ใบกำกับภาษี</h1>
        ...
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          {isEditing ? (
            ...
            <>
              <button onClick={() => {
                const url = `${window.location.origin}/share?type=tax-invoice&id=${docNo}`;
            ...
            <tr>
              <td style={{ border: 'none', padding: 0 }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '40px', marginBottom: '32px', marginTop: '8px' }}>
                  <div>
                    <h3 style={{ fontSize: '1rem', color: 'var(--text-muted)', borderBottom: '1px solid #e2e8f0', paddingBottom: '8px', marginBottom: '12px' }}>ลูกค้า (Customer)</h3>
                    {isEditing ? (
