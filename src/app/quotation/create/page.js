...
import { generatePriceTemplateItems } from '@/lib/price-templates';
      ...
      if (edit === 'true' && id) {
        ...
        try {
          ...
          if (found) {
            ...
            setMapUrl(found.mapUrl || found.googleMapUrl || '');
            setContactName(found.contactName || found.contacts?.[0]?.name || '');
            setContactPhone(found.contactPhone || found.contacts?.[0]?.phone || '');
            if (found.contacts?.length) setCustomerContacts(found.contacts);
            return;
          }
        } catch (e) {
...
${survey.nightShift ? '- ปฏิบัติการรูปแบบพิเศษ: งานช่วงกลางคืน (Night Shift)\n' : ''}${survey.highRise ? '- อุปกรณ์สนับสนุนความสูง: รถกระเช้า / นั่งร้านโรยตัวภายนอกอาคาร\n' : ''}${survey.heavyStains ? '- เทคนิคขจัดคราบฝังลึกพิเศษ\n' : ''}${survey.noWaterElectricity ? '- จัดเตรียมชุดสำรองกระแสไฟและถังน้ำแรงดันสูงเคลื่อนที่\n' : ''}`;
  ...
  const handleGeminiGenerate = () => {
    ...
    const genItems = generatePriceTemplateItems(geminiPrompt);
    setGeminiGeneratedItems(genItems);
    setIsGeminiGenerating(false);
    showToast('สร้างรายการจากเทมเพลตราคากลางเรียบร้อย — กรุณาตรวจสอบและปรับราคาก่อนบันทึก', 'success');
  };
  ...
  const handleApplyGeminiItems = () => {
    ...
    showToast('นำเข้ารายการจากเทมเพลตราคากลางเรียบร้อย', 'success');
  };
  ...
  const handleGeminiCustomerSearch = async () => {
    ...
    try {
      ...
      showToast('ค้นหาข้อมูลบริษัทจากฐานข้อมูลเรียบร้อย', 'success');
    } catch (err) {
      console.error(err);
      setIsGeminiCustomerLoading(false);
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          <button onClick={() => setIsGeminiModalOpen(true)} style={{ backgroundColor: '#eff6ff', color: '#1d4ed8', border: '1px solid #bfdbfe', padding: '10px 16px', borderRadius: '8px', display: 'flex', alignItems: 'center', gap: '8px', fontWeight: '600', cursor: 'pointer' }}>
            <span style={{ fontSize: '16px' }}>📋</span> เทมเพลตราคากลาง
          </button>
        ...
        <div style={{ backgroundColor: 'white', borderRadius: '12px', padding: '40px', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.1)', border: '1px solid var(--border-color)' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '32px' }}>
            <div style={{ backgroundColor: '#f8fafc', padding: '24px', borderRadius: '12px', width: '100%', border: '1px solid var(--border-color)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '80px 1fr 100px 1fr', gap: '12px 16px', alignItems: 'center' }}>
                ...
                <span style={{ fontSize: '0.9rem', color: 'var(--text-muted)' }}>โลเคชัน Maps:</span>
                <input
                  type="url"
                  placeholder="เช่น https://maps.app.goo.gl/xxxxx (ถ้ามี)"
                  value={mapUrl}
                  onChange={e => setMapUrl(e.target.value)}
                  style={{ gridColumn: 'span 3', padding: '6px 10px', border: '1px solid var(--border-color)', borderRadius: '6px', outline: 'none', fontSize: '0.9rem', width: '100%' }}
                />
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', marginTop: '32px', paddingTop: '24px', borderTop: '1px solid var(--border-color)' }}>
            <button
              ...
              style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'white', backgroundColor: '#0f766e', border: 'none', padding: '12px 24px', borderRadius: '8px', fontWeight: '600', cursor: 'pointer' }}
            >
              <Save size={18} /> {isEditMode ? 'บันทึกการแก้ไข' : 'บันทึกใบเสนอราคา'}
            </button>
            <button type="button" onClick={() => window.print()} style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--text-main)', backgroundColor: 'white', border: '1px solid var(--border-color)', padding: '12px 24px', borderRadius: '8px', fontWeight: '600', cursor: 'pointer' }}>
              <Printer size={18} /> พิมพ์ใบเสนอราคา
            </button>
            <button type="button" onClick={() => window.print()} style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'white', backgroundColor: '#10b981', border: 'none', padding: '12px 24px', borderRadius: '8px', fontWeight: '600', cursor: 'pointer' }}>
              <Download size={18} /> พิมพ์ / บันทึก PDF
            </button>
            <button type="button" onClick={() => { const subject = encodeURIComponent(`ใบเสนอราคา ${docNo}`); const body = encodeURIComponent(`เรียน ${customerName}\n\nแนบใบเสนอราคาเลขที่ ${docNo}\n\nขอบคุณครับ`); window.location.href = `mailto:?subject=${subject}&body=${body}`; }} style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'white', backgroundColor: 'var(--primary-color)', border: 'none', padding: '12px 24px', borderRadius: '8px', fontWeight: '600', cursor: 'pointer' }}>
          ...
          <div style={{ backgroundColor: 'white', padding: '32px', borderRadius: '12px', width: '600px', maxWidth: '90%' }}>
            ...
            {!isGeminiCustomerOpen ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <div>
                  <label style={{ display: 'block', fontWeight: '600', marginBottom: '8px', fontSize: '0.9rem', color: 'var(--text-main)' }}>ค้นหาชื่อบริษัทหรือเลขผู้เสียภาษี</label>
                  <input 
                    type="text" 
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.6)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000, padding: '24px' }}>
          <div style={{ backgroundColor: 'white', borderRadius: '16px', padding: '32px', width: '100%', maxWidth: '600px', maxHeight: '90vh', overflowY: 'auto', position: 'relative', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)', border: '2px solid #3b82f6' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px', borderBottom: '2px solid #eff6ff', paddingBottom: '16px' }}>
              ...
              <div>
                <h2 style={{ fontSize: '1.25rem', fontWeight: 'bold', margin: 0, color: '#1e40af' }}>เทมเพลตประเมินราคา</h2>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              <div>
                <label style={{ display: 'block', marginBottom: '8px', fontWeight: '600', fontSize: '0.95rem', color: 'var(--text-main)' }}>อธิบายลักษณะงานที่ต้องการประเมินราคา</label>
                <textarea 
                  rows="4" 
                  value={geminiPrompt} 
              ...
              >
                {isGeminiGenerating ? (
                  <>
                    <span style={{ display: 'inline-block', width: '16px', height: '16px', border: '2px solid #fff', borderTop: '2px solid transparent', borderRadius: '50%', animation: 'spin 1s linear infinite' }} />
                    กำลังสร้างรายการจากเทมเพลต...
                  </>
                ) : '🚀 สร้างรายการราคาจากเทมเพลต'}
              </button>

              {/* แสดงผลการเจน */}
              {geminiGeneratedItems.length > 0 && (
                <div style={{ marginTop: '16px', borderTop: '2px dashed #bfdbfe', paddingTop: '20px' }}>
                  <h3 style={{ fontSize: '1rem', fontWeight: 'bold', color: '#1e40af', marginBottom: '12px' }}>📊 รายการประเมินราคาจากเทมเพลต</h3>
